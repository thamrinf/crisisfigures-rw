...
function drawGeoJson(id, geojsonData) {
	...
	return new Promise((success, error) => {
		...
		if(tmpArrDataDetailMaps.includes(f.properties.adm0_a3) == true){
			...
			_TooltipMap[idxToolTip] = layer.bindPopup(
				tmpContent
			);
		}
...
// bar chart c3js
function barChartC3(arrCountry, arrSeries, categories, idBarChartDataLabel){
	if (tmpChart[idBarChartDataLabel] === undefined) {
		tmpChart[idBarChartDataLabel] = "";
	}
	...
	let hChart = c3.generate({
		...
		size: {
			height: parseInt(arrCountry.length)*30,
		},
		data: {
			x : 'x',
			columns: [
...
async function init() {
	...
	_T_DATA = await d3.csv("assets/csv/dataviz-reliefweb.csv").then((dt) => {
		// for table
	...
	textOptionCountry += "<option value='all'>ALL COUNTRY</option>";
	dataCountry.forEach((val, idx) => {
		textOptionCountry += "<option value='"+idx+"'>"+val.toUpperCase()+"</option>";
	});
...
$(document).ready(function(){
	...
	setTimeout(() => {
		...
		// $(".needBGD").append("<svg></svg>");
		// $(".needBDI").append("<svg></svg>");
		// createBar('.needBGD',18400000,20310000);
		// createBar('.needBDI',18400000,20310000);
		// setTimeout(() => {
